...
public class Api {
	 ...
	 */
	public ListResponse list(String endpoint, Map<String, String> additionalParams) throws ApiException {
		try {
			...
			int totalCount = Integer.valueOf(response.getHeader("X-Total-Count")); 
			
			return new ListResponse(new JSONArray(response.getResponse()), page, totalCount, 
					page * perPage >= totalCount, endpoint, additionalParams, response.getHeaders());
	...
	/**
	 * Shortcut method for {@link Api#list(String, Map)} without additional parameters.
	...
	/**
	 * Returns the next page for the given {@link ListResponse}. Returns null if it
	 * was the last page.
	 * @param response The {@ListResponse} obtained through a call to {@link #list(String, Map)}
	 * or {@link #nextPage(ListResponse)}
	 * @return A {@link ListResponse} with the results
	 */
	 ...
	 * @return A {@link ObjectResponse} with the results, or null if the object does not exist.
	 */
	public ObjectResponse get(String endpoint, int id, Map<String, String> additionalParams) throws ApiException {
		if(id < 0) {
	 ...
	 * @param id The entity id
	 * @return {@link ObjectResponse} with the results, or null if object does not exist
	 */
	...
	/**
	 * Accesses the store endpoint, to obtain the data associated with the store.
	 * @param additionalParams Additional parameters for the request
	 * @return {@link ObjectResponse} with the results
	 */
	public ObjectResponse store(Map<String, String> additionalParams) throws ApiException {
	...
	private ObjectResponse internalGet(String url, Map<String, String> additionalParams) throws ApiException {
		try {
			if(additionalParams == null) {
				additionalParams = Maps.newHashMap();
			}
	...
	/**
	 * Creates an object.
	 * Equivalent to a POST to a certain endpoint.
	...
	/**
	 * Updates an object.
	 * Equivalent to a PUT to a certain endpoint + "/" + the object id.
	 * @param endpoint The endpoint to be accessed. Example: "products", "scripts"
	...
	/**
	 * Deletes an object.
	 * Equivalent to a DELETE to a certain endpoint + "/" + the object id.
	 * @param endpoint The endpoint to be accessed. Example: "products", "scripts"
